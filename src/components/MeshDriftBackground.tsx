...
// Uniforms exact definitions from specifications:
// Colours (low → high): #101010, #F5F5F5, #B0B0B0, #3A3A3A
const U_COLORS = new Float32Array([
...
export const MeshDriftBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.warn('WebGL1 is not supported in this environment.');
      return;
    }
    ...
    // Fullscreen single triangle covering clip-space: (-1,-1), (3,-1), (-1,3)
    ...
    // Resize handling with devicePixelRatio capped at 2
    const updateSize = () => {
      if (!canvas) return;
    ...
    // Animation & RAF Management: pause RAF loop when tab is hidden
    ...
    const render = (now: number) => {
      ...
      // Uniform values matching exact header comment specifications:
      // u_colors[8] (first 4 used)
      if (uColorsLoc) {
        gl.uniform3fv(uColorsLoc, U_COLORS);
      }
      ...
      const elapsedSeconds = Math.max(0, (now - startTime - totalPausedDuration) / 1000.0);
      if (uSceneLoc) {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        if (isHidden) {
          isHidden = false;
          if (pauseTimestamp > 0) {
            totalPausedDuration += performance.now() - pauseTimestamp;
            pauseTimestamp = 0;
          }
        }
        if (animFrameId === null) {
    ...
    const handleContextRestored = () => {
      updateSize();
      if (animFrameId === null && !document.hidden) {
        animFrameId = requestAnimationFrame(render);
      }
    };
